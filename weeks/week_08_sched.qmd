...
How do pdf, CDF, and probability interact with each other?

Joint vs marginal vs conditional: How are we calculating the probability?

What are we actually finding by solving the double integral. In the first example, we found the probability was 1/16 after integrating but what does 1/16 mean in relation to the random variables X and Y?

I think conceptualizing of the domain when the the domain is of the two RVs is not independent (e.g. 0\<x\<y) can be a bit tricky (your demonstration was helpful though). Also from the last slide today, I am not sure I understand the reason that the domain for the conditional probability is x\>=8.
