...
:::
::: ex-cont
Let $f_{X,Y}(x,y)= 2e^{-(x+y)}$, for $0 \leq x \leq y$.
Find $\mathbb{E}[X]$.
:::
:::
...
:::
::: prop-cont
If $X_1, X_2, \ldots X_n$ are continuous RVs and
...
:::
::: prop-cont
If $X$ and $Y$ are independent continuous RVs, and $g$ and $h$
    are functions, then $$\mathbb{E}[g(X)h(Y)] =\mathbb{E}[g(X)]\mathbb{E}[h(Y)]$$
:::
:::
...
:::
::: ex-cont
A machine manufactures cubes with a side length that
